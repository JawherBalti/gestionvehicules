...
const AchatCarburant = ({ navigation, route }) => {
  const [nbrLitres, setNbrLitres] = useState(0);
  ...
  const create = async () => {
    ...
    try {
      ...
      formData.append("litres", nbrLitres);
      formData.append("montant", montant);
      formData.append("id_utilisateur", userInfo.id);
      formData.append("id_vehicule", id);
      formData.append("facture", cloudinaryUrl);

      const response = await axios.post(
        "https://5dc4-41-62-82-125.ngrok-free.app/gestion_vehicules/api/achatCarburant.php",
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        alert("Carburant acheté avec succès");
        setNbrLitres(0);
        setMontant(0);
        setImage(null);
        setImagePreview(null);
      } else {
        alert(response.data.message || "Erreur lors de l'achat");
      }
    } catch (error) {
  ...
  const goBack = () => {
    navigation.navigate("AffichageVehicules", { nextPage: "AchatCarburant" });
  };
    ...
    <>
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.inputContainer}>
          <Text style={styles.pageTitle}>Acheter carburant</Text>
          <Input
            label="Nombre de litres"
            icon="water-outline"
            size={20}
            value={nbrLitres}
            setValue={setNbrLitres}
            placeholder="Entrez la nombre de litres"
            isSecure={false}
