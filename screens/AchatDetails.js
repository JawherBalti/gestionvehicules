...
const AchatDetails = ({ navigation, route }) => {
    ...
    const fetchDetails = async () => {
      ...
      try {
        const response = await axios.post(
          "https://5dc4-41-62-82-125.ngrok-free.app/gestion_vehicules/api/achatDetails.php",
          formData,
          {
            headers: {
    ...
    <>
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.inputContainer}>
          <Text style={styles.pageTitle}>Achats Détails</Text>
        ...
        {achats.length > 0 ? (
          achats.map((achat) => (
            <View key={achat.id} style={styles.achatContainer}>
              <Text style={styles.label}>
                Achat par: {achat.nom} {achat.prenom}
              </Text>
              <Text style={styles.label}>Date: {achat.date}</Text>
              <Text style={styles.label}>Montant: {achat.montant} DT</Text>
              <Text style={styles.label}>Litres: {achat.litres} L</Text>

              <Text style={styles.subLabel}>Factures associées:</Text>
              <ScrollView horizontal style={styles.mediaScroll}>
                {achat.facture.split(",").map((media) => renderMedia(media))}
              </ScrollView>
            </View>
          ))
        ) : (
          <Text style={styles.noDataText}>Aucun achat disponible pour ce véhicule.</Text>
        )}
      </ScrollView>
    </>
  );
